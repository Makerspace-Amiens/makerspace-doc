---
title: Méthodologie de projet
layout: project-home
permalink: /workshops/methodologie-de-projet/
type: workshop
image: /workshops/methodologie-de-projet/hero.jpg
project_slug: methodologie-de-projet
project_image: /workshops/methodologie-de-projet/hero.jpg
project_tags:
  - Gestion de projet
  - Documentation
  - Git & GitHub
description: "Apprendre à cadrer, documenter et gérer un projet de A à Z : du repo GitHub à la présentation finale, en passant par le travail en équipe."
subtitle: De l'idée au projet transmissible

concepts:
  - /workshops/methodologie-de-projet/concepts/cycle-de-vie-projet/
  - /workshops/methodologie-de-projet/concepts/documenter-au-fil-de-leau/
  - /workshops/methodologie-de-projet/concepts/definir-son-besoin/
  - /workshops/methodologie-de-projet/concepts/rechercher-existant-faisabilite/
  - /workshops/methodologie-de-projet/concepts/constituer-organiser-equipe/
  - /workshops/methodologie-de-projet/concepts/communiquer-en-equipe/
  - /workshops/methodologie-de-projet/concepts/gerer-temps-jalons/
  - /workshops/methodologie-de-projet/concepts/prototyper-iterer/
  - /workshops/methodologie-de-projet/concepts/tracer-choix-techniques/
  - /workshops/methodologie-de-projet/concepts/rendre-projet-transmissible/

tutorials:
  - /workshops/methodologie-de-projet/tutorials/creer-repo-template/
  - /workshops/methodologie-de-projet/tutorials/git-github-desktop-vscode/
  - /workshops/methodologie-de-projet/tutorials/syntaxe-markdown/
  - /workshops/methodologie-de-projet/tutorials/readme-efficace/
  - /workshops/methodologie-de-projet/tutorials/gerer-projet-github/
  - /workshops/methodologie-de-projet/tutorials/pull-requests-relecture/
  - /workshops/methodologie-de-projet/tutorials/resoudre-conflit-git/
  - /workshops/methodologie-de-projet/tutorials/journal-de-bord/
  - /workshops/methodologie-de-projet/tutorials/modifier-site-github/
  - /workshops/methodologie-de-projet/tutorials/construire-site-local/
  - /workshops/methodologie-de-projet/tutorials/documenter-carte-electronique/
  - /workshops/methodologie-de-projet/tutorials/documenter-systeme-mecanique/
  - /workshops/methodologie-de-projet/tutorials/documenter-code-firmware/
  - /workshops/methodologie-de-projet/tutorials/documenter-assemblage-montage/
  - /workshops/methodologie-de-projet/tutorials/documenter-tests-resultats/
  - /workshops/methodologie-de-projet/tutorials/poster-video-presentation/
  - /workshops/methodologie-de-projet/tutorials/checklist-cloture-projet/

how_to_guides:
  - /docs/how-to-guides/propriete-intellectuelle-projet/

hardware:
  -

software:
  -

ressources:
  -
---

Bienvenue dans l'atelier **Méthodologie de projet** !

Vous allez apprendre à cadrer, documenter et gérer un projet de bout en bout : créer et faire vivre un repo GitHub, organiser le travail en équipe, documenter au fil de l'eau plutôt qu'au dernier moment, et présenter proprement chaque brique de votre projet (électronique, mécanique, logiciel).

## Ce que vous allez apprendre

- Cadrer un projet : besoin, cahier des charges, cycle de vie
- Documenter en continu, pas seulement à la fin
- Organiser une équipe et communiquer efficacement
- Utiliser Git et GitHub pour versionner, suivre et relire le travail
- Construire et maintenir un site de documentation Jekyll
- Documenter chaque brique du projet (électronique, mécanique, code, tests, montage)
- Clôturer et transmettre un projet correctement

## Pourquoi ce projet ?

Un projet réussi ne se limite pas à un prototype qui fonctionne : il doit pouvoir être compris, repris et transmis. Cet atelier donne la méthode et les outils utilisés au MakerSpace pour que chaque projet étudiant reste exploitable après le rendu — pour vous, pour l'équipe suivante, et pour l'école.
